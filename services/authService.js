...
export const updateUserProfile = async (profileData) => {
  try {
    ...
    if (userDocSnap.exists()) {
      ...
      // Determine if meaningful user info (name/username) is changing
      const isInfoUpdate = 
        (profileData.firstName && profileData.firstName.trim() !== userData.firstName) ||
        (profileData.lastName && profileData.lastName.trim() !== userData.lastName) ||
        (profileData.username && profileData.username.trim() !== userData.username);

      // Only enforce cooldown if name/username is changing
      if (isInfoUpdate && userData.updatedAt) {
        ...
        if (now - lastUpdate < tenMinutes) {
          throw new Error('You can only update your name/username once every 10 minutes.');
        }
      }
    }
